...
% from it, and tools/check_report.py asserts every quoted figure against the
% out-of-fold artefact.  Ablation figures come from tools/compare_pods.py.
...
We measured the achievable ceiling directly, scoring each annotator's own masks
as \emph{the} prediction against every reading of that observation --- self
included, exactly as the scorer treats our single prediction per image.
...
human as predictor, multi-annotator & 0.8307 & 0.7144 & 0.5934 \\
\textbf{our model} & 0.6748 & 0.6175 & 0.4167 \\
\bottomrule
\end{tabular}
\end{center}

Roughly $+0.20$ of SQ and $+0.15$ of RQ remain available. Computing this the
other way --- annotator A against annotator B --- gives 0.6307/0.5211/0.3286 and
makes the model look better than human, because it discards the self-match the
scorer always includes. We flag that explicitly because we made the error
ourselves before catching it.

Annotator disagreement is \emph{not} the constraint. Out-of-fold PQ on
single-annotator observations, where an exact match is achievable by
construction, is \textbf{0.4163}, against \textbf{0.4169} on multi-annotator
ones. If disagreement bound us we would score far better where there is none.

The striking finding is how rigid SQ is. Mean matched IoU reaches $\approx 0.67$
by epoch six and does not move for the remaining 24 epochs, while PQ swings by
0.13 through recognition alone. Every run we have trained --- both encoders, all
five folds, 20- and 30-epoch schedules, fp16 and bf16, four GPU models --- lands
in 0.63--0.68, and exactly one matched pair in 2501 exceeds IoU 0.90.

This matters because the metric is unusually sensitive to SQ: shifting every
instance's IoU by a constant gives a near-linear \textbf{1.06 PQ per unit of mean
IoU} (Table~\ref{tab:headroom}), so $+0.05$ IoU would be worth more than
converting \emph{every} one of our 321 near-miss instances ($+0.048$). Masks are
only 8\% too large by area, far too little to explain an IoU of 0.67, so the
error is in \emph{where} a boundary sits rather than how large the mask is.

Yet none of the four interventions in Section~\ref{sec:ablations} moved it. The
honest statement is therefore narrower than we would like: SQ carries both the
most headroom and the most leverage, and we have not found the variable that
controls it. The interventions that did not work are reported in full, because
between them they eliminate most of the obvious hypothesis space.
...
Each treatment was trained against its own baseline on the same GPU, in the same
precision, under the same code version and seed, so that the two arms differ in
exactly one variable. Significance is judged against a measured noise floor
rather than asserted: four identical baselines trained across four machines span
\textbf{0.0072} in PQ and 0.0056 in SQ.

\begin{table}[t]
\centering
\caption{Paired ablations, fold 0. Noise floor $\Delta\mathrm{PQ} = 0.0072$.}
\begin{tabular}{lrrl}
\toprule
treatment & $\Delta$PQ & $\Delta$SQ & verdict \\
\midrule
EfficientNet-B4 vs B0 (bf16) & $-0.0005$ & $+0.0047$ & within noise \\
spine head, weight 0.3 & $-0.0060$ & $+0.0038$ & within noise \\
boundary-weighted BCE & $-0.0012$ & $+0.0005$ & within noise \\
...
\textbf{The recorded B4 penalty was an artefact of precision.} B4 stood at
$-0.021$ against B0. That measurement was taken on a Turing T4 in fp16, where B4
overflowed and the gradient scaler silently discarded the affected steps, so it
could not distinguish a worse model from an undertrained one. Repeated on Ampere
in bf16, which carries fp32's exponent range, the penalty is $-0.0005$: it does
not exist. B4 is therefore a candidate \emph{ensemble member} rather than a
replacement, and that is how we use it.

\textbf{The spine head does not reduce fragmentation.} It is the most promising
idea available on paper --- the organisers permit the metadata explicitly and
name structural continuity as a core challenge --- but one-to-many counts are
unchanged (42 against 42) while precision falls from 0.6405 to 0.6177 on 29
additional predictions.

Together with the training curves, these rule out training length, numerical
precision, hardware, encoder capacity, edge-weighted loss and full-resolution
decoder input as explanations for the SQ ceiling. We report them because a
negative result measured against a noise floor is worth more than an untested
idea described as promising.
...
\textbf{The empty-observation fallback does not pay.} Ten of 707 observations
emit nothing, which is certainly wrong: no training reading contains zero
filaments. We added the rescue as a \emph{tunable axis} rather than as a
correction applied on the strength of the argument, and the tuner rejected it ---
refitting with the axis available scores 0.4147 on held-out data against 0.4167
without it.
